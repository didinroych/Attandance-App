...
userRouter.get('/api/users/schedule/date', classSessionController.getClassScheduleByDateController); //done
//GET /api/users/schedule/weekly?startDate=YYYY-MM-DD (optional)
userRouter.get('/api/users/schedule/weekly', classSessionController.getWeeklyScheduleController); //done
//GET /api/users/schedule/academic-period/:academicPeriodId
userRouter.get('/api/users/schedule/academic-period/:academicPeriodId', classSessionController.getScheduleByAcademicPeriodController); //done
...
userRouter.get('/api/teacher/classes', classSessionController.getClassListClassTeacherController); //done
//Get all sessions for a specific class schedule
userRouter.get('/api/teacher/class/:classScheduleId/sessions', classSessionController.getClassSessionsListController); //done
//Create a new attendance session | @body { classScheduleId: number, date: string }
userRouter.post('/api/teacher/sessions', classSessionController.createClassSessionController); //done
//Get detailed session information with all attendances
userRouter.get('/api/teacher/session/:id', classSessionController.getClassSessionController); //done
//Update attendance records for a session | @body { attendances: [{ attendanceId: number, status: string, notes: string }] }
userRouter.patch('/api/teacher/session/:id/attendance', classSessionController.updateClassSessionController); //done
// Update teacher profile
userRouter.patch('/api/teacher/profile', userController.updateTeacher); //belom
...
userRouter.patch('/api/admin/users/:id/role', adminController.updateRoleUser); //belom


userRouter.get('/api/users/academic-periode', academicPeriodeCon.getAcademicPeriodCon); //belom
